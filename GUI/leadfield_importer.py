...
# Resolve FieldEvaluator from repository scripts/modules when this file resides in GUI/
FieldEvaluator = None  # type: ignore
try:
    # Try standard modules package if available on sys.path
    from modules.field_metrics import FieldEvaluator as _FE  # type: ignore
    FieldEvaluator = _FE
except Exception:
    try:
        # Add scripts/modules to sys.path relative to this file
        CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
        REPO_ROOT = os.path.dirname(CURRENT_DIR)
        MODULES_DIR = os.path.join(REPO_ROOT, 'scripts', 'modules')
        if MODULES_DIR not in sys.path:
            sys.path.insert(0, MODULES_DIR)
        from field_metrics import FieldEvaluator as _FE  # type: ignore
        FieldEvaluator = _FE
    except Exception:
        try:
            # Last resort: dynamic import by file location
            import importlib.util
            fm_path = os.path.join(MODULES_DIR, 'field_metrics.py')
            if os.path.exists(fm_path):
                spec = importlib.util.spec_from_file_location('field_metrics', fm_path)
                if spec and spec.loader:
                    mod = importlib.util.module_from_spec(spec)
                    spec.loader.exec_module(mod)
                    FieldEvaluator = getattr(mod, 'FieldEvaluator', None)
        except Exception:
            FieldEvaluator = None
